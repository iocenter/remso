...
%}
%{
	changes by codas:  take into account sr in gradient calculation
%}
...
function varargout = relperm(s, opt, varargin)
   ...
   if nargout > 1,
      sUSat = s(:,1) > opt.sr(1) & s(:,1) < 1-(opt.sr(2));
      null = zeros(size(s1));
      varargout{2} = [ kwm(1) * n(1) .* s1 .^ (n(1) - 1).* sUSat, ...
                       null, null                       , ...
                       kwm(2) * n(2) .* s2 .^ (n(2) - 1).* sUSat] ./ den;
   end

   if nargout > 2,
      a = n .* (n - 1);
      varargout{3} = [ kwm(1) * a(1) .* s1 .^ (n(1) - 2).* sUSat, ...
                       kwm(2) * a(2) .* s2 .^ (n(2) - 2).* sUSat] ./ den;
   end
end
