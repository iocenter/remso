function [state,A,x,b] = solveIncompFlowLocal(state, g, s, fluid, varargin)
...
%}
   ...
   if do_solve,
      ...
      x2        = solver(A, b);
        ...
        AM  = [cell2mat(A)                               
           A{2}', sparse(size(A{2},2),size(A{2},2)), sparse(size(A{2},2),size(A{3},2))
           A{3}', sparse(size(A{3},2),size(A{2},2)), sparse(size(A{3},2),size(A{3},2))];
     
       xM =  AM\cell2mat(b');
       x = mat2cell(xM,[size(A{1},2) size(A{2},2) size(A{3},2)],size(xM,2))';
       x{4} = [];
      
      assert(norm(cell2mat(x2(1:3)')-cell2mat(x(1:3)'))<1e-4,'This norm is getting too high' );
...
function [A, b, dF, dC] = build_system(state, g, s, w, bc, src, fluid, opt)
   ...
   if ~isempty(opt.rhs),
      ...
      assert (all(cellfun(@(u,v) all(size(u,1) == size(v,1)), b, opt.rhs)));
      b = opt.rhs;
   end
