...
f_bc    = -sparse(obj.partials(curStep).v');

cellNo  = rldecode(1:G.cells.num, diff(G.cells.facePos), 2) .';
S.C     = sparse(1:numel(cellNo), cellNo, 1);
f_bc    = f_bc - dt*( bsxfun(@times,f_w(A.j),( S.C*(bsxfun(@times,invPV,l_s))) ) ) ...
               + dt*( S.C*( bsxfun(@times,(-f_w.*dQMinus + dQPluss),( bsxfun(@times,invPV,l_s) ) )) );
...
for wellNr = 1 : numel(W)
    ...
    W(wellNr).S.RHS.h = sparse( size(W(wellNr).S.RHS.h,1) ,size(W(wellNr).S.RHS.f,2));
    inx = inx + numCells;
end
