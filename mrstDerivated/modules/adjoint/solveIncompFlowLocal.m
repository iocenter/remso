...
Option to compute numerical gradients with respect to the saturations in forward mode
%}
...
if do_solve,
    ...
    if ~isempty(rhsS)
        for k = 1:size(rhsS,2)
            ...
            pert = 0.0000001;
            resSolP = state;
            resSolP.s = resSolP.s + rhsS(:,k)*pert;

            
            [AP, bP, dFP, dCP] = build_system(resSolP, g, s, opt.wells, ...
                    opt.bc, opt.src, fluid, opt);
