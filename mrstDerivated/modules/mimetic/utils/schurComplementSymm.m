...
Tricks to deal with multiple rhs
%}
...
%
ncell = size(g,1);               % Number of cells in model
numD = size(D,2);
...
M = CtBIDf(:, 1:numD);         % == C' * inv(B) * D
Mt= D'*BI*C;
S = DtBIDf(:, 1:numD);         % == D' * inv(B) * D

g = g - CtBIDf(:, numD+1:end);         % == g - C'*inv(B)*f
h = h - DtBIDf(:, numD+1:end);         % == h - D'*inv(B)*f
...
S = S - MtLIMg(:, 1:numD);     % == S - M.'*inv(L)*M
h = MtLIMg(:,numD+1:end) - h;          % == M.'*inv(L)*g - h
...
%   L p = g + M*lam
%
press = LIMg(:, numD+1:end) + LIMg(:, 1:numD)*lam;
...
%   B v = f + C*p - D*lam
%
flux  = BIDf(:, numD+1:end) - BIDf(:, 1:numD)*lam + BI*C*press;
