function [converged CNV MB] = getConvergence(state, eqs, fluid, system, dt)
	...
	%  Check in addition the convergence of the well - control convergence!
    %  TODO: think about a more appropriate tolerance for this!
...
%     [CNVO CNVW CNVG]
    ...
    %%% TODO: OW - systems  check for others
    
    
    CWB = max([abs(eqs{3}.val);abs(eqs{4}.val);abs(eqs{5}.val)]);
    converged_CWB = CWB < tol_cnv;
...
%     figure(2); plot(pv); drawnow;
    ...
    converged = converged_MB & converged_CNV & converged_CWB;
