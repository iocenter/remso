...
%
% Modification by Codas: remove the requirement of a wellSol structure on
% the state0
% include the cdpCalc option
...
% Modification by codas:  Numerical value not needed, just the number of
% wells (maybe not even!)
    ...
    if ~opt.resOnly,
        if ~opt.reverseMode,
            ...
            [p0, sW0, x0, zw, zw, zw, zw] = ...
                ...
                zeros(size(W)) , zeros(size(W)) , ...
                zeros(size(W)) , zeros(size(W)));                 %#ok
