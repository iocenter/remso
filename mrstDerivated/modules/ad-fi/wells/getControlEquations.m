function eq = getControlEquations(sol, pBH, q_s, status, mix_s, model, varargin)
opt = struct('netWells',      []);
...
% wells controlled by the network
if ~isempty(opt.netWells)    
    eq(opt.netWells) = val(opt.netWells) - val(opt.netWells); %% TODO: implement this residual for net controlled wells
end
