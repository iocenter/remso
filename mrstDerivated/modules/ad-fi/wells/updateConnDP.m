function sol = updateConnDP(W, sol, b, rMax, rhos, model,varargin)
...
%{
  ...
  With the approximation the fluid hydrostatic pressure is calculated with constant density depending on compi.  Thererfore, with gas wells this approximation might be very bad.
%}

opt = struct('approxForExactJacs',      false);
...
for k = 1:numel(sol);
    ...
    if opt.approxForExactJacs
        if strcmp('OW',model)
            ...
            warning('Be carefull Jacobian might be wrong');
        end
    else
        warning('Be carefull Jacobian might be wrong');
    end
