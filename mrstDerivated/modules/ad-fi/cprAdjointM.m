...
%
   ...
   if ~isempty(g)
       warning('Check that the extension to multiple RHS is correct for the gas case too!')
   ...
   if opt.iterative
       ...
       lambda_cells = opt.directSolver(A',rhs);
   end
   %%
   ...
   if any(fl) ~= 0
      ...
      dispif(mrstVerbose,'Used %d GMRES iterations\n', its(2));
   end
end
