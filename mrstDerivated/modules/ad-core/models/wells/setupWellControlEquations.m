...
Use ADI' to take the jacobian w.r.t the controls
...
setToZeroRate = and(double(val) ==0, ~cellfun(@(x)strcmp('bhp',x), type));
disabledWells  = ~status;

if isa(pBH,'ADI') && isa(val,'ADI')
eq = pBH; %just to initialize to whatever class pBH is
    val = double(val);
elseif isa(val,'ADI')
    eq = pBH + 0 * val(1); % use pBH to preserve right dimensions    
else
    eq = pBH;
end
