...
limit dpMaxAbs to 50*barsa
octave is unable to parse input provided a model object: do it explicitly

Tighten the convergence tolerances for the well equations
TODO: These seem to be very tight! relax if convergence problmes are found
...
methods
    ...
    function model = ReservoirModel(G, varargin)
        ...
        model.toleranceCNV = 1e-4;
        model.toleranceMB = 1e-7;
        model.toleranceWellBHP = 0.01*Pascal;
        model.toleranceWellRate = 0.0001/day;
        ...
        model.wellmodel = WellModel('allowWellSignChange',opt.allowWellSignChange,...
                                    'allowCrossflow',opt.allowCrossflow,...
                                    'allowControlSwitching',opt.allowControlSwitching);
        
        % Gravity defaults to the global variable
        model.gravity = gravity(); %#ok
        if ~isempty(varargin) && numel(varargin) >= 2 && strcmp(varargin{1},'inputdata') % provisory solution for octave: parse varargin explicitly
            model.inputdata = varargin{2};
            varargin = varargin(3:end);
        end
    ...
    function [Jacobian] = getDrivingForcesJacobian(model,state0, state, dt, drivingForces)
        ...
        Wclean = model.wellmodel.W;
        model.wellmodel.W = W;
        
        %Clean ADI's
        model.wellmodel.referencePressure = double(model.wellmodel.referencePressure);
        model.wellmodel.bfactors = ADI.cellADI2cellDouble(model.wellmodel.bfactors);
        model.wellmodel.mobilities = ADI.cellADI2cellDouble(model.wellmodel.mobilities);
        
        [wellEqs,ctrlEqs,cq_s] = model.wellmodel.assembleEquations(state.wellSol,q_s,pBH,model);
        
        model.wellmodel.W = Wclean;
        
        varDims = cellfun(@(x)size(x,2),ctrlEqs.jac);
        Jacobian = mat2cell(sparse(sum(eqDims),sum(varDims)),eqDims,varDims);
        if isa(cq_s{1},'ADI')
            wc    = vertcat(W.cells);
            for k = 1:numel(cq_s)
                cq_s{k} = -cq_s{k}; %% the source term is included with a negative sign in equationsOilWater;            
                jacs = cq_s{k}.jac;
                for j = 1:numel(jacs)
                    [Jacobian{k,j}(wc,:)] = jacs{j};  %% incidence of the well source equations in the residual equations
                end
            end
        end
        if isa(wellEqs{1},'ADI')
            for k = 1:numel(wellEqs)
                jacs = wellEqs{k}.jac;
                [Jacobian{end-1-numel(wellEqs)+k,:}] = jacs{:};
            end
        end
        jacs = ctrlEqs.jac;
        [Jacobian{end,:}] = jacs{:};        
    ...
    function wellSol = updateWellSol(model, wellSol, problem, dx, drivingForces, wellVars) %#ok
        ...
        for i = 1:numel(wellVars)
            ...
            for j = 1:numel(wellSol)
                wellSol(j).(wf) = (wellSol(j).(wf) + dv(j)).*wellSol(j).status;
            end
        end
    end
...
% --------------------------------------------------------------------%
    ...
    function [varargout] = toWellSol(model,wellSolVector,Well,varargin)
        ...
        if nargin < 4
            wellSol = struct('name',{Well.name},'sign',{Well.sign});
        else
            state = varargin{1};
