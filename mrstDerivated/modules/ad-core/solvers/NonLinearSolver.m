...
optional initial solution guess
%}
    ...
    methods
        ...
        function [state, report, ministates] = solveTimestep(solver, state0, dT, model, varargin)
            ...
            optGuess = struct('initialGuess',[]);
            
            [drivingForces,extraOpts] = merge_options(drivingForces, varargin{:});
            
            [optGuess] = merge_options(optGuess, extraOpts{:});
            ...
            if isempty(optGuess.initialGuess)
                state = state0;
            else
                state = optGuess.initialGuess;
            end
...
function [state, converged, failure, its, reports] = solveMinistep(solver, model, state, state0, dt, drivingForces)
    ...
    if converged
        state = model.updateAfterConvergence(state0, state, dt, drivingForces);
    end
end
