...
REMSO is tightly interfaced to MRST (currently MRST-2013b) and exploits the structure of the module ad-fi (Automatic Differentiation - Fully Implicit). Functionalities within MRST are modified to enable REMSO to calculate simulations in parallel and gradient computations.
...
REMSO is under continuous development and test. The MASTER branch contains a version that was considerably tested and which performance results were submitted for publication. During the revision process that branch will maintain the same code. Meanwhile, other branches, for ex. 2014a, will merge MRST updates and new features.
