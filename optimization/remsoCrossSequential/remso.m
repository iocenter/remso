...
for k = 1:opt.max_iter
    ...
    if k>1  % Do not perform updates if the watchdog is active!
        ...
        if hInit
            M = [];
            [M,S,Y, skipping,sTy] = dampedBFGSLimRestart(M,y,s,nru,S,Y,'scale',opt.BFGSRestartscale,'it',k,'m',opt.BFGSRestartmemory,'allowDamp',opt.allowDamp,'condT',opt.condT);
            hInit = skipping;
        else
            [ M,S,Y,~,sTy ] = dampedBFGSLimRestart(M,y,s,nru,S,Y,'scale',opt.BFGSRestartscale,'it',k,'m',opt.BFGSRestartmemory,'allowDamp',opt.allowDamp,'condT',opt.condT);
        end
    ...
    elseif opt.feasibleProjection && xi == 1 && l == 0 
        
        %% Merit function definition
        merit = @(f,dE,varargin) l1merit(f,dE,1,varargin{:});
        duF = cellfun(@(z)z*0,du,'UniformOutput',false);
        
        
        [err0] = lineFunctionWrapper(0,...
            x,...
            v,...
            u,...
            ax,...
            av,...
            duF,...
            simFunc,@zeroTarget,merit,...
            'gradients',false,...
            'plotFunc',opt.plotFunc,...
            'plot',opt.plot,...
            'withAlgs',withAlgs,...
            'debug',opt.debug,...
            'simVars',simVars,...
        ...
        for kf = 1:4
            ...
            if kf == 1
                ...
                axT = cellfun(@(dz)l*dz,ax,'UniformOutput',false);
                if withAlgs
                    avT = cellfun(@(dz)l*dz,av,'UniformOutput',false);
                end                
            end

            [~,dxF]  = checkBounds( ldx,axT,udx,'chopp',true,'verbose',true);
            if withAlgs
                [~,dvF]  = checkBounds(ldv,avT,udv,'chopp',true,'verbose',true);
            end            
            
            [errF,~,vars,simVars ] = ...
                lineFunctionWrapper(1,...
                x,...
                v,...
                u,...
                dxF,...
