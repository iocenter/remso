function [ w,stepY ] = computeCrossTerm(x,u,v,s,ax,av,as,gbarZ,ss,obj,mudx,mudu,mudv,muds,lbx,lbv,lbs,ubx,ubv,ubs,varargin)
...
if stepY > 0
    stepY = min(stepY,maxStepCalc(x,v,ax,av,lbx,lbv,ubx,ubv,tol));
end
...
if stepY == 0  % try to compute in the negative side
    ...
    if stepY > 0
    	axN = uMinus(ax);
        avN = uMinus(av);
        stepY = min(stepY,maxStepCalc(x,v,axN,avN,lbx,lbv,ubx,ubv,tol));
    end
    stepY = -stepY;
end
...
if stepY == 0
    ...
    while ~all(convergedR) && abs(stepY) > opt.minStep
        ...
        if stepY == 1
            ...
            if ~isempty(opt.xs) && ~isempty(opt.vs)
                ...
                xRG = buildGuessC(opt.xs,x,ax,stepY);

                vRG = buildGuessC(opt.vs,v,av,stepY);
        ...
        try                  
            [xsR,vsR,s2R,~,convergedR,simVarsR,uslicedR] = simulateSystem_R(xR,u,vR,ss,'gradients',false,'guessX',xRG,'guessV',vRG);
            if ~all(convergedR)
                stepY = stepY/2;
            end
        catch
    ...
    if all(convergedR)
        ...
        [gradUY,convergedR] = simulateSystemZ_R(u,xR,vR,ss,gbarRdx,'simVars',simVarsR);
