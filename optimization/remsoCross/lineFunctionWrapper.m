function [ f,df,vars,simVars,debugInfo ] = lineFunctionWrapper(stepL,x0,v0,u0,dx,dv,du,simF,target,meritF,jobSchedule,varargin)
%
...
if opt.fwd && opt.gradients
    ...
    [tarL,JacTar] = target(x,u,v,'gradients',true,'xRightSeeds',dx,'uRightSeeds',du,'vRightSeeds',dv);
    
    % merit function evalutaion
    if withAlgs
        [f,JacF,debugInfo] = meritF(tarL,{eX;eV},'gradients',true,'fRightSeeds',JacTar.J,'dERightSeeds',{jeX;jeV},'debug',opt.debug);
    else
        [f,JacF,debugInfo] = meritF(tarL,{eX},'gradients',true,'fRightSeeds',JacTar.J,'dERightSeeds',{jeX},'debug',opt.debug);
    end
...
if ~isempty(opt.plotFunc) && opt.plot
        opt.plotFunc(bringVariables(vars.x,jobSchedule),...
            vars.u,...
            bringVariables(vars.v,jobSchedule),...
            bringVariables(eX,jobSchedule));
end
