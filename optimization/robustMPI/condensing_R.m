...
xd = opt.xd;
if isempty(xd)
    xd = createEmptyDistributedVar(jobSchedule);
end
vd = opt.vd;
if isempty(vd)
    vd = createEmptyDistributedVar(jobSchedule);
end
...
if computeNullSpace && computeCorrection
    if ~isempty(uRightSeeds)
        ...
        dsdax = gopMPI('+',dsdax,jobSchedule);
        dsdav = gopMPI('+',dsdav,jobSchedule);
        %end
        ...
        %spmd
            ...
            Asv = catAndSum(Asv);
            Asv = gopMPI('+',Asv,jobSchedule);
            
            Asx = catAndSum(Asx);
            Asx = gopMPI('+',Asx,jobSchedule);
        %end
