...
leftSeedsGiven=true;
if iscell(leftSeed)
	if numel(leftSeed) == numel(jobSchedule.work2Job(jobSchedule.my_rank+1));
    % do no thing
	else
    leftSeed = distributeVariablesMPI(leftSeed,jobSchedule);
	end
else
    leftSeed = createEmptyDistributedVar(jobSchedule);
