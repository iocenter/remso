...
for i = 1:varN
    ...
    if debug
        ...
        debugInfo.ineq = max(debugInfo.ineq,dineqi);
    end
...
if opt.gradients
    if ~(size(opt.fRightSeeds,1)==0)
        ...
        for i = 1:numel(dE)
            ...
            ubActi = ubActC{i};
            lbActi = lbActiC{i};
            bERightSeedsi = repmat({1},size(taui));
            
            
            
            JdE = [JdE,cellfun(@eqLinePenaltyJac,dEi,dERightSeedsi,'UniformOutput',false)'];
            JbE = [JbE,cellfun(@ineqLinePenaltyJac,taui,ubActi,lbActi,bERightSeedsi,'UniformOutput',false)'];
        end
