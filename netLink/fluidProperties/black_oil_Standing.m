...
if p <= p_b % saturated oil 
    ...
    R_s = R_sb;
    B_ob = oil_form_vol_fact_Standing(R_s,rho_g_sc,rho_o_sc,T); % oil formation volume factor
    % at bubble point pressure, m^3/m^3
    rho_g_100 = rho_g_Vazquez_and_Beggs(p_sc,rho_g_sc,rho_o_sc,T_sc); % gas density at 100 psi,
    % kg/m^3
    c_o = compres_Vazquez_and_Beggs(p,R_s,rho_g_100,rho_o_sc,T); % oil compressibility, 1/Pa
