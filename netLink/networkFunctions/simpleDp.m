function dp =  simpleDp(E, qoE, qwE, qgE, pV, hasSurfaceGas,f_work,Z)
...
if any(~flag_rate)
    if any(flag_rate)
        dp(flag_rate) =  simpleDp(E(flag_rate), qoE(flag_rate), qwE(flag_rate), qgE(flag_rate), pV(flag_rate), hasSurfaceGas, f_work, Z);
    end
    return
end
