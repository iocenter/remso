...
minProd = struct('ORAT',2*meter^3/day,  'WRAT',-inf*meter^3/day,  'GRAT',-inf*meter^3/day,'BHP',-inf*barsa);
...
%% Impose constraint on the first and last integration step only! SS

for k = setdiff(1:totalPredictionSteps,[lastControlSteps',1,lastControlSteps(1:end-1)'+1])
    lbv{k}= -inf(size(lbv{k}));
    ubv{k} = inf(size(ubv{k}));
end
